...
class CheckKeyMissingAction(BaseAction):
    ...
    def execute_on_file_logic(self, post, file_path: str) -> tuple[bool, str]:
        ...
        if not key_to_check:
            self.logger(f"FEHLER (CheckKeyMissingAction): Key nicht spezifiziert für {base_name}.")
        ...
        if key_to_check not in post.metadata:
            ...
            # self.logger(f"Info: In '{base_name}': Key '{key_to_check}' vorhanden (Wert: '{value}').")
            return False, f"Key '{key_to_check}' vorhanden"