...
class DeleteFilesByKeyValueAction(BaseAction):
    ...
    def execute_on_file_logic(self, post, file_path: str) -> tuple[bool, str]:
        ...
        if key_to_match is None or value_to_match is None:
            self.logger(f"FEHLER (DeleteFilesByKeyValueAction): Key oder Value nicht spezifiziert für {base_name}.")
            return False, "Key oder Value nicht spezifiziert"

        # Prüfen, ob der Key existiert und den gewünschten Wert hat
        if key_to_match in post.metadata and str(post.metadata[key_to_match]) == str(value_to_match):
            if self.is_dry_run:
                self.logger(f"[DryRun] In '{base_name}': Datei würde gelöscht (Match: '{key_to_match}':'{value_to_match}').")
                return True, f"würde Datei löschen (Match: '{key_to_match}':'{value_to_match}')"
            else:
                try:
                    os.remove(file_path)
                    self.logger(f"GELÖSCHT: Datei '{base_name}' ({file_path}) (Match: '{key_to_match}':'{value_to_match}').")
                    return True, f"Datei gelöscht (Match: '{key_to_match}':'{value_to_match}')"
                except Exception as e:
                    self.logger(f"FEHLER beim Löschen der Datei '{base_name}' ({file_path}): {e}")
                    return False, f"Fehler beim Löschen: {e}"
        else:
            # Bedingung nicht erfüllt, keine Aktion
            return False, f"Bedingung ('{key_to_match}':'{value_to_match}') nicht erfüllt"
